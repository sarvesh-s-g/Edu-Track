...
class StudentCourse(models.Model):
    ...
    def get_cie(self):
        # Calculate CIE as: (Internal1 + Internal2 + Internal3 + Event1 + Event2 + SEE) / 2 (rounded up)
        required_names = [
            'Internal test 1',
            'Internal test 2',
            'Internal test 3',
        ...
        for name in required_names:
            try:
                mk = self.marks_set.get(name=name)
                total += mk.marks1 or 0
            except Marks.DoesNotExist:
                # Missing mark treated as 0
                total += 0
        return math.ceil(total / 2)

    def get_see(self):
        """Return Semester End Exam marks1 (or 0 if missing)."""
        try:
            mk = self.marks_set.get(name='Semester End Exam')
            return mk.marks1 or 0
        except self.marks_set.model.DoesNotExist:
            return 0
...
def create_attendance(sender, instance, **kwargs):
    if kwargs['created']:
        try:
            ...
            # Skip attendance creation if AttendanceRange doesn't exist yet (e.g., during data loading)
            pass
...
def create_marks_class(sender, instance, **kwargs):
    if kwargs['created']:
        for name in test_name:
            # Check if already exists to avoid duplicates during fixture loading
            if not MarksClass.objects.filter(assign=instance, name=name[0]).exists():
